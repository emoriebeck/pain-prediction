...
print(jobid)
args <- read.table("scripts/cluster/args/biscwit.csv"
...
biscwit_fun <- function(sid, outcome, group, time){
  ...
  save(tune_res, file = sprintf("05-results/02-biscwit/01-tuning-models/%s_%s_%s_%s.RData",
                                sid, outcome, group, time))
  ...
  p <- tune_res %>%
    ...
    theme_classic()
  ggsave(p, file = sprintf("05-results/02-biscwit/02-tuning-figures/%s_%s_%s_%s_%s.png",
  ...
  save(final_biscwit
       , file = sprintf("05-results/02-biscwit/03-final-training-models/%s_%s_%s_%s.RData",
                        sid, outcome, group, time))
  ...
  save(final_coefs, best_biscwit,
       file = sprintf("05-results/02-biscwit/07-final-model-param/%s_%s_%s_%s.RData",
                      sid, outcome, group, time))
  ...
  save(final_metrics
       , file = sprintf("05-results/02-biscwit/06-final-model-performance/%s_%s_%s_%s.RData",
                        sid, outcome, group, time))
  ...
  save(final_var_imp
       , file = sprintf("05-results/02-biscwit/05-variable-importance/%s_%s_%s_%s.RData",
                        sid, outcome, group, time))
  ...
  #                        , sid, outcome, group, time)) 
  # ggsave(p_roc, file = sprintf("05-results/02-biscwit/04-roc-curves/%s_%s_%s_%s.png",
  #              sid, outcome, group, time)
  #        , width = 5, height = 5)
...
biscwit_fun(args[,1], args[,2], args[,3], args[,4])